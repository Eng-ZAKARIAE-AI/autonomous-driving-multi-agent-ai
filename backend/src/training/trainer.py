...
class Trainer:
    def __init__(self, config: Dict[str, Any], algorithm: str = 'ppo'):
        ...
        if self.algorithm == 'baseline':
            self.agent = BaselineAgent(config if isinstance(config, dict) else config._config)
