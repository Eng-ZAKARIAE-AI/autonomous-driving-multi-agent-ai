...
class CarlaGymEnv:
    ...
    def connect(self) -> bool:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                # Use get_world() to connect to whatever map CARLA already has loaded
                # (avoids the ~60s map-switch that load_world() triggers)
                self.world = self.client.get_world()
                current_map = self.world.get_map().name
                print(f"[OK] Connecte a CARLA -- carte active : {current_map}")
                ...
                print("[OK] Connexion CARLA etablie", flush=True)
                return True
            except Exception as exc:
                print(f"[ERR] CARLA initialization failed (attempt {attempt}/{retries}): {exc}", flush=True)
                if attempt < retries:
                    time.sleep(delay)
                else:
                    print("[ERR] CARLA could not be reached after retries."
                          " Start the simulator manually.", flush=True)
                    return False
    ...
    def _auto_launch_carla(self) -> None:
        ...
        if launch_command:
            try:
                ...
                print(f"[WARN] Failed to auto-launch CARLA with command: {exc}")
        elif carla_root:
            ...
            if os.path.exists(executable):
                try:
                    ...
                    print(f"[WARN] Failed to auto-launch CARLA executable: {exc}")
            else:
                print(f"[WARN] CARLA executable not found at {executable}")
        else:
            print("[WARN] CARLA auto-launch requested but no launch_command or carla_root provided.")

    def _spawn_ego(self) -> None:
        blueprint_library = self.world.get_blueprint_library()
        bps = list(blueprint_library.filter(self.config['carla']['ego_filter']))
        if not bps:
            raise RuntimeError(f"No blueprint for filter '{self.config['carla']['ego_filter']}'")
        ...
        for sp in spawn_points:
            ...
            if self.ego is not None:
                # autopilot is OFF by default for new actors — no set_autopilot() needed
    ...
    def _spawn_traffic(self) -> None:
        num_vehicles = int(self.config['carla']['traffic_vehicles'])
        if num_vehicles <= 0:
            return
        ...
        if self.traffic_manager is None:
            try:
                ...
                print(f"[WARN] Traffic Manager indisponible ({exc}) — pas de NPC", flush=True)
                return

        blueprint_library = self.world.get_blueprint_library()
        vehicle_blueprints = blueprint_library.filter('vehicle.*')
        spawn_points = self.world.get_map().get_spawn_points()
        num_vehicles = min(num_vehicles, max(0, len(spawn_points) - 1))
    ...
    def _setup_camera_sensor(self) -> None:
        # Use perception_camera config (640x640 for YOLO quality).
        # Falls back to deprecated 'camera' key if perception_camera is absent.
        cam_cfg = self.config.get('perception_camera', self.config.get('camera', {}))
        blueprint_library = self.world.get_blueprint_library()
    ...
    def _get_observation(self) -> Dict[str, Any]:
        ...
        detections = self.perception.detect(self.camera_image)
    ...
    def get_annotated_frame(self) -> Optional[np.ndarray]:
        """Return the latest camera frame with YOLO bounding boxes drawn (if enabled)."""
        if self.camera_image is None:
            return None
        return self.perception.draw_overlay(self.camera_image, self.last_detections)
